    ...
    xhr.onreadystatechange = function () {
        ...
        if (xhr.readyState === XMLHttpRequest.DONE) {
            ...
            if (xhr.status === 200) {
                ...
                // Funzione per rimuovere i duplicati in base al nome
                function rimuoviDuplicati(array) {
                    var uniqueNames = [];
                    var uniqueIds = [];

                    array.forEach(function(autore) {
                        if (!uniqueNames.includes(autore.nome) && !uniqueIds.includes(autore.id)) {
                            uniqueNames.push(autore.nome);
                            uniqueIds.push(autore.id);
                        }
                    });

                    // Filtra gli autori originali in base agli id unici
                    return array.filter(function(autore) {
                        return uniqueIds.includes(autore.id);
                    });
                }

                // Applica la funzione per rimuovere i duplicati
                autori = rimuoviDuplicati(autori);
                var materiali = rimuoviDuplicati(materiali);
                var tecniche = rimuoviDuplicati(tecniche);
                var misure = rimuoviDuplicati(misure);
...
function rimuoviGruppoMisure(containerId) {
    ...
    else if (gruppoMisureElements.length == 2) {
        var ultimoElemento = gruppoMisureElements[gruppoMisureElements.length - 1];
        ultimoElemento.parentNode.removeChild(ultimoElemento);
        bottoneRimuovi = document.getElementById('removeButtonGruppiMisure');
        bottoneRimuovi.style.display = 'none';
                ...
                <div class="field">
                    <div class="form-group misure" id="misureGroup${id}1">
                        ...
                        <option value="" selected>---</option>
                            ...
                            <option value="Profondita">Profondita</option>
                            <option value="Spessore">Spessore</option>
                        ...
                        <select name="Unita" id="Unita${id}1">
                            ...
                            <option value="hg">hg (ettogrammi)</option>
...
async function gatherData() {
    try {
        ...
        formData["bibliografia"] = document.getElementById("bibliografia").innerHTML ? document.getElementById("bibliografia").innerHTML : "";
        formData["altraBibliografia"] = document.getElementById("altrabibliografia").innerHTML ? document.getElementById("altrabibliografia").innerHTML : "";
